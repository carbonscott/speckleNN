...
class SPIMosaicDataset(Dataset):
    """
    SPI mosaic are collected from multiple datasets specified in the input csv
    ...
    def form_mosaic(self, panels, **kwargs):
        ''' Stitch images in panels to form a single mosaic.
        '''
        # Get the size of each image...
        size_y, size_x = panels[0].shape

        # Make room for stiching along y-axis...
        img_mosaic = np.zeros((size_y * len(panels), size_x), dtype = np.float32)
    ...
    def filter_panels(self, panels, **kwargs):
        panels_filtered = panels
        if self.panels_ordered is not None:
            panels_filtered = [ panels[i] for i in self.panels_ordered ]

        return panels_filtered


    def get_mosaic_and_label(self, idx):
        # Read image...
        ...
        with h5py.File(h5_path, 'r') as fh:
            panels = fh.get(self.KEY_TO_IMG)[id_frame]

        # Filter panels...
        panels = self.filter_panels(panels)
        ...
        if self.trans is not None:
            for panel in panels:
                panels_trans.append(self.trans(panel))

        # If no trans is available, just use the original values???
        if not len(panels_trans): panels_trans = panels

        # Form a mosaic...
        img_mosaic = self.form_mosaic(panels_trans) if self.IS_MOSAIC else panels_trans

        return img_mosaic, label


    def __getitem__(self, idx):
        img, label = self.get_mosaic_and_label(idx)
...
class Siamese(SPIMosaicDataset):
