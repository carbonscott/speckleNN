...
trans             = None
alpha             = 0.02
scan_rng          = np.linspace(1e-2, 1e2, 100)
...
for enum_pdb, pdb in enumerate(pdb_list):
    ...
    # [USER INPUT]
    for enum_photon, photon_scale in enumerate(scan_rng):
        ...
        pdb_photon_res_dict[pdb].append([photon_scale, res_dict])

        acc = ConfusionMatrix(res_dict).get_metrics(1)[0]
        print(f"Working on {pdb}, {photon_scale}, {acc}...")
