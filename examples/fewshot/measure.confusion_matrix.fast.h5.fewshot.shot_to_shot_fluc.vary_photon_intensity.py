...
from image_preprocess_faulty_sq_with_fluc_for_validate import DatasetPreprocess
...
for enum_pdb, pdb in enumerate(pdb_list):
    ...
    # Apply shot to shot fluc upfront...
    dataset_list = [ (random_scale_function(img), label, metadata) for img, label, metadata in dataset_list ]

    # Scan through a list of shot to shot fluc setting...
    ## for enum_photon, photon_exponent in enumerate(np.linspace(-2,2,1)):
    for enum_photon, photon_exponent in enumerate(np.linspace(-2,2,40)):
        photon_scale = 10 ** photon_exponent

        # Increase the photon intensities...
        dataset_list = [ (img * photon_scale, label, metadata) for (img, label, metadata) in dataset_list ]
        ...
        print(f"Working on {pdb}, {photon_exponent}...")


fl_pdb_photon_res_dict = 'pdb_photon_res_dict.pickle'
