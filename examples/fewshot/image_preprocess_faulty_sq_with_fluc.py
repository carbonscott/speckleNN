...
class DatasetPreprocess:
    ...
    def __init__(self, img, fl_shot_to_shot_fluc = None): 
        self.img = img
        self.fl_shot_to_shot_fluc = fl_shot_to_shot_fluc
    ...
    def apply_random_patch(self):
        # Random patching...
        num_patch = 10
        size_patch_y, size_patch_x = 12, 12
    ...
    def apply_random_shift(self):
        frac_y_shift_max = 0.05
        frac_x_shift_max = 0.05
    ...
    def apply_shot_to_shot_fluc(self):
        random_scale_function = None

        if self.fl_shot_to_shot_fluc is not None:
            # Add shot to shot fluctuation to data_train and data_validate...
            # Load photon count and probability
            photon_count_list, prob_list = np.load(self.fl_shot_to_shot_fluc)
            ...
            logger.info(f"TRANS : Apply shot to shot fluctuation.")

        self.random_scale_function = random_scale_function
    ...
    def trans(self, img, **kwargs):
        ...
        # Apply shot to shot fluc...
        if getattr(self, "random_scale_function", None) is not None:
            img = self.random_scale_function(img)
