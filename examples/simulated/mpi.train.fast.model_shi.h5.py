...
fl_dataset    = '5VFR.pnccd.pickle'    # Raw, just give it a try
...
dataset_train.mpi_cache_dataset()
dataset_validate.mpi_cache_dataset()

dataset_train_modified    = DatasetModifierForModelShi(dataset_train)
dataset_validate_modified = DatasetModifierForModelShi(dataset_validate)
...
if mpi_rank == 0:
    ...
    # [[[ MODEL ]]]
    # Config the model...
    num_pos    = len([ data for data in dataset_train_modified if data[1] == 1])
    num_neg    = len(dataset_train_modified) - num_pos
    pos_weight = num_neg / num_pos
    config_model = Config( name = "Model", encoder = encoder, pos_weight = pos_weight)
    ...
    config_train = ConfigTrainer( path_chkpt        = path_chkpt,
                                  ...
                                  lr                = lr, 
                                  tqdm_disable      = True)
    trainer = SimpleTrainer(model, dataset_train_modified, config_train)
    ...
    validator = SimpleValidator(model, dataset_validate_modified, config_validator)
