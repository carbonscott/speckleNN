...
fl_log   = f'{timestamp}.validate.query.generalizability'
fl_log   = f'{fl_log}.log'
drc_log  = 'logs'
...
true_label = '1'
true_labeled_item_dict = { k : v for k, v in res_dict[true_label].items() if k == true_label }
...
with open(path_csv, 'w') as csvfile:
    ...
    for k, v in true_labeled_item_dict.items():
        for false_item in v:
