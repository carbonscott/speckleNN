...
with open(path_csv, 'r') as fh:
    lines = csv.reader(fh)
    ## num_imgs = (sum(1 for line in lines) - 1) * 2
    true_pair_list = [ line for line in lines ]

select_pair_num = 10
true_pair_random_list = random.sample(true_pair_list, select_pair_num)

for idx_line, line in enumerate(true_pair_random_list):
    ...
    # Load support image...
    exp, run, event_idx, _ = tag_support.split()
    ...
    img_support = psana_reader_dict[basename].get(event_idx)
...
fl_pdf  = f'{timestamp}.{true_label}.true'
disp_manager = VizFalse(img_ary, title = 'true labeled', figsize = (8.5, 5.7))
## disp_manager = VizFalse(img_ary, title = 'true labeled', figsize = (8.5, 10))
disp_manager.show()
