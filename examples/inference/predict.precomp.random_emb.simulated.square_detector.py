...
size_sample_train = 400
frac_train        = 0.01

size_precompute_sample_per_label = 20
...
emb_rnd_dict = {}
for k, seqi_list in dataset.label_seqi_simpleset_dict.items():
    # Randomly select one vector...
    idx_rnd_list = random.sample(seqi_list, size_precompute_sample_per_label)

    # Save the selected embedding...
    embs_selected = embs[idx_rnd_list]
    emb_rnd_dict[k] = torch.mean(embs_selected, dim = 0)    # Mean along sample dim
